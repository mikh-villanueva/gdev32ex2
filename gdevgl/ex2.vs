...
void main()
{
    ...
    {
        tangentWorld = abs(worldSpaceNorm.y) < 0.999
            ? cross(vec3(0.0f, 1.0f, 0.0f), worldSpaceNorm)
            : cross(vec3(1.0f, 0.0f, 0.0f), worldSpaceNorm);
    }

    tangentWorld = normalize(tangentWorld - dot(tangentWorld, worldSpaceNorm) * worldSpaceNorm);
